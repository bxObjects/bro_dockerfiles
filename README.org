...
evolve together. Alongside that pair, the per-leaf bash scripts
(=build.bash=, =verify.sh=, =run.bash=, =docker-compose*.yml=) are a
*permanent standalone path*: they need nothing but docker/podman, so this
repo stays fully usable on hosts where BISOS is not installed --- notably
the old RedHat VMs. Both paths are maintained at parity. The
...
Within this tree today, =ftoBranchProc.spcs= is planted at all nine
branch directories (=debian/=, =debian/1{2,3}/=, and each of the six
profile dirs), and every branch wires the walker to =bisos.dockerProc=
via =leafExamples=/=leafProcessors=. Leaves are detected
*definitionally* --- by the presence of a planted =dockerProc.spcs= or
=podmanProc.spcs= --- so no per-leaf =_tree_= marker is needed and a
walk from =debian/= reaches all six leaves. Branch-side =_tree_= /
=_treeProc_= markers are so far planted only at
=debian/12/confined/=; the other eight will follow. See
[[file:/bisos/git/auth/bxRepos/bisos-pip/fileObj/README.org][=bisos.fileObj/README.org=]]
for the walker; see the =pycs/spcs/README.org= link above for the
full architecture.

For how this repo relates to =bisos.dockerProc=, =bisos.fileObj= and the
=rawBuild*= reconstruction commands, see
[[#related-pieces-----where-this-fits][Related Pieces --- Where This Fits]] below.
...
| Yellow rectangle                   | *Bash path (standalone)* --- =build.bash= / =verify.sh= / =docker compose= / =run.bash=. No BISOS required. |
| Green rectangle                    | *=.spcs= path (framework)* --- =dockerProc.spcs= / =podmanProc.spcs= via =bisos.dockerProc=. |
...
- *Two build/verify paths (bottom-left).* Every leaf supports two ways
  ...
  =bisos.dockerProc= install required, keeping the repo usable on hosts
  without BISOS. The =.spcs= path (=dockerProc.spcs= for docker leaves,
  =podmanProc.spcs= for rootless-sysd leaves) derives all parameters
  from the leaf's directory path via =bisos.dockerProc=. Both are
  permanent and maintained at parity --- which path you use depends on
  whether BISOS is available on the host, not on which is newer.
...
- [[#related-pieces-----where-this-fits][Related Pieces --- Where This Fits]]
  - [[#two-ways-to-drive-more-than-one-leaf][Two Ways to Drive More Than One Leaf]]
- [[#scope-of-this-repo][Scope of This Repo]]
  - [[#this-tree-is-the-test-matrix-for-rawbisos-reconstruction-container-based][This tree is the test matrix for RawBisos-Reconstruction (container-based)]]
...
* Related Pieces --- Where This Fits
  :PROPERTIES:
  :CUSTOM_ID: related-pieces-----where-this-fits
  :END:

This repo is one of *four related pieces*. Each has a distinct job and none
subsumes another. This repo holds the *content*; the other three are machinery
that operates on it.

| Piece                                | Kind        | Job                                                                                                                                                                       |
|--------------------------------------+-------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| =bro_dockerfiles= (*this repo*)        | BRO (repo)  | *Container image specifications.* The six-leaf matrix --- Dockerfiles, systemd units, per-leaf assets, and the =~/raw-bisos= payload baked into each image.                  |
| [[https://github.com/bisos-pip/dockerProc][=bisos.dockerProc=]]                  | pip package | *Container framework.* The planted =dockerProc.spcs= / =podmanProc.spcs=, path-derived parameters, and the =containerProc_*= verbs that build / run / verify one leaf.       |
| [[https://github.com/bisos-pip/fileObj][=bisos.fileObj=]]                      | pip package | *Generic tree walker.* FTO marker conventions plus =ftoBranchProc.spcs=, which applies one verb across every leaf beneath a branch. Domain-agnostic --- knows nothing about containers. |
| [[https://github.com/bisos/raw][=/bisos/core/raw/rebuild=]]           | git repo    | *RawBisos-Reconstruction --- BISOS's CI/CD.* =rawBuildBox.cs= / =rawBuildKvm.cs= / =rawBuildContainer.cs=: one uniform entry point for rebuilding Raw-BISOS on metal, in a KVM guest, or in a container. |

The first two are *jointly designed* and evolve together --- specification and
framework, described as a pair throughout this README. The other two are
*consumers*: =bisos.fileObj= supplies the walker this tree plants at its
branches, and =rawBuildContainer.cs= locates a leaf here and subprocess-invokes
its planted =.spcs=, exactly as its siblings shell out to
=sysCharGuestMaterialize.sh= and =sysCharBoxMaterialize.sh=.

** Two Ways to Drive More Than One Leaf
   :PROPERTIES:
   :CUSTOM_ID: two-ways-to-drive-more-than-one-leaf
   :END:

Two facilities operate above a single leaf. They are not competing --- they
answer *different questions*:

- *=bisos.fileObj= walker --- location-addressed, breadth.* /One verb, every
  leaf beneath where you are standing./ Selection is *position in this tree*:
  stand at =debian/13/= and the verb reaches both Debian 13 leaves; stand at
  =debian/= and it reaches all six.

  #+BEGIN_SRC bash
  cd debian
  ./ftoBranchProc.spcs -i fto_forwardToLeaves --cmndName=containerProc_instanceVerify
  #+END_SRC

- *=rawBuildContainer.cs= --- parameter-addressed, depth.* /One leaf, an
  ordered sequence of steps./ Selection is =freshDeb= plus =platformModel=
  (=confined= / =sysd-priv= / =sysd-podman=), and each step maps onto one
  =containerProc_*= verb run in the leaf's directory.

  #+BEGIN_SRC bash
  rawBuildContainer.cs -i procContainer --freshDeb="13" --platformModel="sysd-priv" \
      --steps='["delete", "up", "installRawBisos", "verifyUp"]'
  #+END_SRC

The =rawBuild*= family deliberately does not use the walker: its sibling
commands target a KVM guest by =bpoId= and a box by IP address, and have no
tree to walk. A tree is a container-side property that only one of the three
platform kinds has.
...
** This tree is the test matrix for RawBisos-Reconstruction (container-based)
...
full-release validation). See [[file:/bisos/git/auth/bxRepos/bisos-pip/dockerProc/README.org][bisos.dockerProc/README.org]] for the
...
*Containers are one of three reconstruction substrates.* Above this tree,
=/bisos/core/raw/rebuild= provides the platform-independent entry point:
=rawBuildContainer.cs= for containers, =rawBuildKvm.cs= for KVM guests,
=rawBuildBox.cs= for raw metal --- one uniform face for Raw-BISOS
reconstruction regardless of what is being reconstructed. Its
=procContainer= command drives *this* tree's leaves through an ordered
=steps= pipeline:

: rawBuildContainer.cs -i procContainer --freshDeb="13" --platformModel="sysd-priv" --steps='["delete", "up", "installRawBisos", "verifyUp"]'

See [[file:/bisos/git/auth/bxRepos/bisos/raw/rebuild/README.org][=raw/rebuild/README.org=]] and
[[#two-ways-to-drive-more-than-one-leaf][Two Ways to Drive More Than One Leaf]] above.
...
result. Both are permanent peers, maintained at parity. Which one you use is
determined by *what the host has available*, not by which is newer.

- *Bash path (standalone --- no BISOS required).* =./build.bash= to build,
  =./verify.sh= to smoke-test, =docker compose up -d= / =./run.bash= to run.
  Needs nothing but docker/podman. This is what keeps =bro_dockerfiles=
  usable as a standalone repo, and it is the *only* path available on hosts
  without BISOS --- notably the old RedHat VMs, where =bisos.dockerProc=
  cannot be installed.

- *=.spcs= path (framework-driven --- requires =bisos.dockerProc=).*
  =./dockerProc.spcs -i containerProc_imageBuild= (or =./podmanProc.spcs=
  for rootless-sysd leaves), =-i containerProc_instanceUp=,
  =-i containerProc_instanceVerify=, =-i containerProc_instanceDelete=, etc.
  Requires =pipx install bisos.dockerProc=. Derives engine, profile, ports,
  and base image from the leaf's directory path --- no per-leaf
  configuration. Cmnds are organized by noun: =image*= for the built
  artefact, =instance*= for the running container, =fullClean= for a
  combined teardown.

The two paths coexist permanently. Same Dockerfile, same layers, same running
container --- the =.spcs= wrapper mostly just calls the same underlying
=docker= / =podman= commands with the right arguments derived from the path.
Changes to one path should be mirrored into the other so the two stay at
parity.
...
- =bisos.dockerProc= (companion pip package --- the framework half of this
  pair): https://github.com/bisos-pip/dockerProc ---
  [[file:/bisos/git/auth/bxRepos/bisos-pip/dockerProc/README.org][local README]]
- =bisos.fileObj= (the generic FTO tree walker planted at this tree's
  branches): https://github.com/bisos-pip/fileObj ---
  [[file:/bisos/git/auth/bxRepos/bisos-pip/fileObj/README.org][local README]]
- =/bisos/core/raw/rebuild= (RawBisos-Reconstruction across boxes, guests and
  containers): https://github.com/bisos/raw ---
  [[file:/bisos/git/auth/bxRepos/bisos/raw/rebuild/README.org][local README]]
- =.spcs= architecture (three-box layered figure):
  [[file:/bisos/git/auth/bxRepos/bisos-pip/pycs/spcs/README.org][=bisos.pycs/spcs/README.org=]]
