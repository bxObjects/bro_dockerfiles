...
Before building, verify host readiness with the host-scoped check shipped in
the =bisos.dockerProc= package:
=/bisos/git/auth/bxRepos/bisos-pip/dockerProc/py3/bin/hostPodmanVerify.sh=
(prints GO/NO-GO). Note: rootless Podman needs a real user session --- log in
as the user directly (not =su=), or crun's systemd cgroup manager fails with an
...
Run =dockerProc/py3/bin/hostPodmanVerify.sh= to check these before building.
...
*Local-only workflow.* These images are meant to be built and used *locally*,
not pushed to DockerHub. Always use =./build.bash -d= (local build); the bare
