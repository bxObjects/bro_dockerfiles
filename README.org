...
- *=bisos.dockerProc= (bottom-right).* The external pip package. It
  plants =dockerProc.spcs= into confined + privileged leaves and
  =podmanProc.spcs= into rootless-sysd leaves. Once planted, each
  =.spcs= file is invoked directly from its leaf directory.
...
- *=.spcs= path (current, framework-driven).* =./dockerProc.spcs -i
  containerProc_build= (or =./podmanProc.spcs= for rootless-sysd leaves),
  =-i containerProc_verify=, =-i containerProc_composeUp=, =-i
  containerProc_run=. Requires =pipx install bisos.dockerProc=. Derives
  engine, profile, ports, and base image from the leaf's directory path ---
  no per-leaf configuration.
...
#+BEGIN_SRC bash
cd debian/12/confined/vnc/xfce/bisos_deb12-fresh
./dockerProc.spcs -i containerProc_build           # with layer cache
./dockerProc.spcs -i containerProc_build --noCache="true"
#+END_SRC
...
#+BEGIN_SRC bash
cd debian/12/privileged/vnc/xfce/bisos_deb12-sysd
./dockerProc.spcs -i containerProc_build
#+END_SRC
...
#+BEGIN_SRC bash
cd debian/12/rootless-sysd/vnc/xfce/bisos_deb12-rootless-sysd
./podmanProc.spcs -i containerProc_build
#+END_SRC

Run =./dockerProc.spcs= or =./podmanProc.spcs= with no arguments in any leaf
to see the full menu (=containerProc_composeUp=, =containerProc_run=,
=containerProc_verify=, =containerProc_status=, =containerProc_clean=).
